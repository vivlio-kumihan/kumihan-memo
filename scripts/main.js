...
const removeItemFn = (cartItemDOM, productObj, removedEl) => {
...
// イベントリスナーを登録した関数を定義。
const handleCartItem = (cartItemDOM, productObj, targetEl) => {
  ...
  minusBtn.addEventListener("click", () => {
    cart.forEach(cartItem => {
      if (cartItem.name === productObj.name) {
        cartItem.quantity > 1
          ? updateCartItemQuantity(cartItemDOM, cartItem, -1)
          : removeItemFn(cartItemDOM, productObj, targetEl);
      }
    });
  });
...
const productAddBtnStateFn = (elem, changeText, bool) => {
...
const afterReloadeGenerateCartDOM = () => {
  cart.forEach(productObj => {
    ...
    productAddToCartBtnEl &&
      productAddBtnStateFn(productAddToCartBtnEl, "In Cart", true);
...
addToCartBtns.forEach((addToCartBtnEl) => {
  ...
  addToCartBtnEl.addEventListener("click", () => {
    ...
    if (!isInCart) {
      ...
      // カートに追加ボタンの表示を変える。
      productAddBtnStateFn(addToCartBtnEl,"In Cart", true);
      // localStorageへ保存。
      saveCartToLocalStorage();
